...
def create_memory_tools(store):
    ...
    @tool
    def save_memory(content: str, category: str = "general", tags: Optional[str] = None) -> str:
        """保存长期记忆
    ...
    @tool
    def search_memory(query: str, category: Optional[str] = None) -> str:
        """搜索长期记忆
        ...
        try:
            ...
            for item in items:
                ...
                results.append(
                    f"【{value.get('category', 'general')}】{value.get('content', '')}\n"
                    f"  标签: {', '.join(value.get('tags', []))} | "
                    f"创建: {value.get('created_at', '')[:10]}"
                )
    ...
    @tool
    def list_memories(category: Optional[str] = None, limit: int = 10) -> str:
        """列出所有长期记忆
        ...
        try:
            ...
            for item in items:
                ...
                results.append(
                    ...
                    f"  ID: {item.key} | 标签: {', '.join(value.get('tags', []))}"
                )
    ...
    @tool
    def update_memory(memory_id: str, content: Optional[str] = None, category: Optional[str] = None, tags: Optional[str] = None) -> str:
        """更新长期记忆
        ...
        try:
            ...
            store.put(namespace, memory_id, updated_data)
    ...
    def delete_memory(memory_id: str) -> str:
        ...
        try:
            namespace = ("memories",)
            success = store.delete(namespace, memory_id)
