...
class ASRConfigService:
            ...
            if row["config_data"]:
                try:
                    ...
                    if row["is_configured"]:
                        # 已配置：脱敏敏感字段
                        provider_data["config"] = self._mask_sensitive_fields(config)
                    else:
                        # 未配置：返回完整模板（供前端显示表单）
    ...
    def _mask_sensitive_fields(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, field_config in config.items():
            if not isinstance(field_config, dict) or "type" not in field_config:
                logger.warning(f"配置字段 {key} 格式异常")
                masked[key] = field_config
                continue
            
            # 带元数据格式
            masked[key] = field_config.copy()
            
            # 如果是敏感字段（password类型或sensitive=True），脱敏value
            is_sensitive = (
                field_config.get("type") == "password" or 
                field_config.get("sensitive", False)
            )
            
            if is_sensitive and "value" in masked[key] and masked[key]["value"]:
                value = str(masked[key]["value"])
                if len(value) > 8:
                    masked[key]["value"] = value[:4] + "******" + value[-4:]
                else:
                    masked[key]["value"] = "******"
