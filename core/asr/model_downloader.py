...
async def download_sensevoice_model_async(
    target_dir: Path | None = None, precision: str = "both"
) -> Path:
    ...
    DOWNLOAD_PROGRESS.update(
        {"is_downloading": True, "progress": 0.0, "error": None, "precision": precision}
    )

    # 确定需要下载的文件清单
    files_to_download = ["tokens.txt"]
    if precision in ("int8", "both"):
        files_to_download.append("model_q8.onnx")
    if precision in ("fp32", "both"):
        files_to_download.append("model.onnx")
            ...
            for filename in files_to_download:
                ...
                async with client.stream("GET", url) as resp:
                    if resp.status_code != 200:
                        raise RuntimeError(
                            f"无法访问文件 {filename}: HTTP {resp.status_code}"
                        )
                    ...
                    with open(target_path, "wb") as f:
                        async for chunk in response.aiter_bytes(
                            ...
                            DOWNLOAD_PROGRESS["progress"] = round(
                                min(99.9, (downloaded_bytes / total_bytes) * 100), 1
                            )
...
def _check_model_exists(model_dir: Path, precision: str = "both") -> bool:
    ...
    if not (model_dir / "tokens.txt").exists():
        return False
    if precision in ("int8", "both") and not (model_dir / "model_q8.onnx").exists():
        return False
    if precision in ("fp32", "both") and not (model_dir / "model.onnx").exists():
        return False
    return True
...
def get_model_info(model_dir: Path | None = None) -> dict:
    ...
    info = {
        "exists": _check_model_exists(model_dir, precision="int8")
        or _check_model_exists(model_dir, precision="fp32"),
...
def delete_model_assets(model_dir: Path | None = None, precision: str = "all"):
    if model_dir is None:
        from core.config import get_settings
    ...
    if (
        not (model_dir / "model_q8.onnx").exists()
        and not (model_dir / "model.onnx").exists()
    ):
        shutil.rmtree(model_dir)
...
def reset_download_progress():
    ...
    global DOWNLOAD_PROGRESS
    DOWNLOAD_PROGRESS.update(
        {"is_downloading": False, "progress": 0.0, "error": None, "precision": None}
    )
