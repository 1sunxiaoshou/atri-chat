...
class SenseVoiceASR:
    ...
    def __init__(self, model_dir: str | None = None, num_threads: int = 4):
        self.num_threads = num_threads
        self.model_dir = (
            Path(model_dir) if model_dir else Path("data/models/sensevoice")
        )
    ...
    def _init_recognizer(self, use_int8: bool, language: str):
        ...
        try:
            ...
            model_file = str(
                self.model_dir / ("model_q8.onnx" if use_int8 else "model.onnx")
            )
    ...
    def _check_model_files(self, use_int8: bool) -> bool:
        ...
        if not (self.model_dir / "tokens.txt").exists():
            return False
        if use_int8 and not (self.model_dir / "model_q8.onnx").exists():
            return False
        if not use_int8 and not (self.model_dir / "model.onnx").exists():
            return False
        return True

    def _load_audio(self, audio: bytes | str | Path) -> tuple:
        """从内存或磁盘安全解析音频并重采样"""
        try:
    ...
    def transcribe(
        self, audio: bytes | str | Path, language: str = "auto", use_int8: bool = False
    ) -> str:
        """语音转文字（同步）"""
        ...
        try:
            ...
            raise RuntimeError(f"推理失败: {str(e)}") from e

    async def transcribe_async(
        self, audio: bytes | str, language: str = "auto", use_int8: bool = False
    ) -> str:
