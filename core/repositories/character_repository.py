...
from core.db import Character, CharacterMotionBinding, Motion
...
class CharacterRepository(BaseRepository[Character]):
    ...
    def get(self, id: str) -> Character | None:
        """根据 ID 获取角色"""
        return self.db.query(Character).filter(Character.id == id).first()

    def get_with_relations(self, id: str) -> Character | None:
        ...
        return (
            ...
            .options(
                ...
                joinedload(Character.motion_bindings).joinedload(
                    CharacterMotionBinding.motion
                ),
            )
    ...
    def get_avatar_expressions(self, character_id: str) -> list[str]:
        ...
        try:
            ...
            logger.warning(
                f"角色 {character_id} 的形象表情数据解析失败，使用默认列表",
                exc_info=True,
            )
            pass
    ...
    def get_character_motions(
        self, character_id: str, category: str | None = None
    ) -> list[Motion]:
        """获取角色绑定的动作列表
    ...
    def list(self, skip: int = 0, limit: int = 100, **filters) -> list[Character]:
        """列出角色
        ...
        if filters.get("enabled_only", False):
            query = query.filter(Character.enabled)

        # 按名称搜索
        if "search" in filters and filters["search"]:
            query = query.filter(Character.name.ilike(f"%{filters['search']}%"))
    ...
    def update(self, id: str, **data) -> Character | None:
        """更新角色"""
    ...
    def get_by_provider_config_id(
        self, provider_config_id: int
        ...
        return (
            self.db.query(Character)
            .filter(Character.primary_provider_config_id == provider_config_id)
            .all()
        )
