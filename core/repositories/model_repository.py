...
class ModelRepository(BaseRepository[Model]):
    ...
    def get_by_provider_and_model(
        self, provider_config_id: int, model_id: str
    ) -> Model | None:
    ...
    def list(self, skip: int = 0, limit: int = 100, **filters) -> list[Model]:
        """列出模型
        ...
        if "provider_config_id" in filters:
            query = query.filter(
                Model.provider_config_id == filters["provider_config_id"]
            )
        ...
        if filters.get("has_vision"):
            query = query.filter(Model.has_vision)
        if filters.get("has_audio"):
            query = query.filter(Model.has_audio)
        if filters.get("has_video"):
            query = query.filter(Model.has_video)
        if filters.get("has_reasoning"):
            query = query.filter(Model.has_reasoning)
        if filters.get("has_tool_use"):
            query = query.filter(Model.has_tool_use)
        if filters.get("has_document"):
            query = query.filter(Model.has_document)
        if filters.get("has_structured_output"):
            query = query.filter(Model.has_structured_output)
    ...
    def list_by_provider(
        self, provider_config_id: int, enabled_only: bool = False
    ) -> builtins.list[Model]:
        """列出指定供应商的所有模型
    ...
    def update(self, id: str, **data) -> Model | None:
        """更新模型"""
        model = self.get(id)
        if not model:
            return None
    ...
    def update_by_provider_and_model(
        self, provider_config_id: int, model_id: str, **data
    ) -> Model | None:
