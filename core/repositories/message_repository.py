...
class MessageRepository(BaseRepository[Message]):
    ...
    def list(self, skip: int = 0, limit: int = 100, **filters) -> list[Message]:
        """列出消息
    ...
    def list_by_conversation(
        ...
        Returns:
            消息列表（按时间正序）
        """
        return self.list(skip=skip, limit=limit, conversation_id=conversation_id)
    ...
    def update(self, id: str, **data) -> Message | None:
        """更新消息"""
        message = self.get(id)
        if not message:
            return None
