...
class ProviderRepository(BaseRepository[ProviderConfig]):
    ...
    def get(self, id: int) -> ProviderConfig | None:
        """根据内部整数 ID 获取供应商"""
        return self.db.query(ProviderConfig).filter(ProviderConfig.id == id).first()
    ...
    def get_by_name(self, name: str) -> ProviderConfig | None:
        ...
        Returns:
            供应商配置对象
        """
        return self.db.query(ProviderConfig).filter(ProviderConfig.name == name).first()
    ...
    def update(self, id: int, **data) -> ProviderConfig | None:
        """更新供应商"""
    ...
    def update_by_name(self, name: str, **data) -> ProviderConfig | None:
        """根据显示名称更新供应商"""
