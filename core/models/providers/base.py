...
class BaseProvider:
    ...
    def get_parameters_schema(
        self, model_id: str, has_reasoning: bool
    ) -> dict[str, Any]:
        """获取适用于特定模型的参数 Schema"""
    ...
    def _list_ollama_models(
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                return [
                    self.get_model_info(m["name"], provider_config)
    ...
    def get_model_info(
        self, model_id: str, provider_config: ProviderConfig
    ) -> ProviderModelInfo:
        """模型探测逻辑：基于 init_chat_model(...).profile 获取模型能力"""
        from langchain.chat_models import init_chat_model

        info = ProviderModelInfo(
            model_id=model_id, type=ModelType.CHAT, parameters={}, meta={}
        )
        ...
        try:
            ...
            if any(x in mid for x in ["o1", "qwq", "deepseek-r1", "thought"]):
                info.has_reasoning = True

        return info

    def create_model(
        self, model_config: ModelConfig, provider_config: ProviderConfig, **kwargs
    ) -> Any | None:
