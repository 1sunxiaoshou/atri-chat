...
class ProviderModelInfo(BaseModel):
    ...
    nickname: str | None = Field(default=None, description="模型昵称")
    ...
    has_reasoning: bool = Field(
        default=False, description="是否支持深度推理 (Reasoning)"
    )
    ...
    max_output: int | None = Field(default=None, description="最大输出token数")
    parameters: dict[str, Any] = Field(
        default_factory=dict, description="模型默认初始化参数"
    )
    meta: dict[str, Any] = Field(
        default_factory=dict, description="存储模型完整的元数据 (Profile)"
    )
...
class ConfigField(BaseModel):
    ...
    sensitive: bool = Field(
        default=False, description="是否为敏感字段（如密码、API密钥）"
    )
...
class ProviderMetadata(BaseModel):
    ...
    discovery: str = Field(
        default="openai", description="发现协议: openai, ollama, google, none"
    )
    config_fields: list[ConfigField] = Field(
        default_factory=list, description="可配置字段列表"
    )
    provider_options_schema: dict[str, Any] | None = Field(
        default=None, description="供应商特定参数的 Schema"
    )
    common_parameters_schema: dict[str, Any] | None = Field(
        default=None, description="通用模型参数的 Schema"
    )
