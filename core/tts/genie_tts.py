...
class GenieTTS(TTSBase):
    ...
    async def _ensure_character_loaded(self):
        ...
        logger.info("加载 Genie TTS 角色模型", extra={"character": self.character_name})
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info("角色模型加载成功", extra={"message": result.get("message")})
    ...
    async def _ensure_reference_audio_set(self):
        ...
        logger.info("设置 Genie TTS 参考音频", extra={"character": self.character_name})
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{self.api_url}/set_reference_audio", json=payload
            )
            response.raise_for_status()
            result = response.json()
            logger.info("参考音频设置成功", extra={"message": result.get("message")})
    ...
    async def test_connection(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.api_url}/load_character", json=payload
                )
                response.raise_for_status()
                result = response.json()
            ...
            return {"success": False, "message": f"无法连接到服务: {self.api_url}"}
        except httpx.TimeoutException:
            return {"success": False, "message": f"连接超时: {self.api_url}"}
    ...
    async def unload_character(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(
                    f"{self.api_url}/unload_character", json=payload
                )
                response.raise_for_status()
                result = response.json()
                ...
                logger.info("角色模型已卸载", extra={"character": self.character_name})

                return {"success": True, "message": result.get("message", "角色已卸载")}
        except Exception as e:
    ...
    async def clear_reference_cache(self) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(
                    f"{self.api_url}/clear_reference_audio_cache"
                )
                response.raise_for_status()
                result = response.json()
                ...
                return {"success": True, "message": result.get("message", "缓存已清除")}
        except Exception as e:
