...
class Avatar(Base):
    ...
    # VRM 特性
    available_expressions: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
...
class Motion(Base):
    ...
    tags: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # JSON 数组
    ...
    def __repr__(self):
        return (
            f"<Motion(id={self.id}, name={self.name}, duration_ms={self.duration_ms})>"
        )
...
class TTSProvider(Base):
    ...
    def __repr__(self):
        return (
            f"<TTSProvider(id={self.id}, type={self.provider_type}, name={self.name})>"
        )
...
class VoiceAsset(Base):
    ...
    # 关系
    provider: Mapped["TTSProvider"] = relationship(
        "TTSProvider", back_populates="voices"
    )
    characters: Mapped[list["Character"]] = relationship(
        "Character",
    ...
    def __repr__(self):
        return (
            f"<VoiceAsset(id={self.id}, name={self.name}, provider={self.provider_id})>"
        )
...
class Model(Base):
    ...
    has_structured_output: Mapped[bool] = mapped_column(
    ...
    provider: Mapped["ProviderConfig"] = relationship(
        "ProviderConfig", back_populates="models"
    )
    characters: Mapped[list["Character"]] = relationship(
...
class Character(Base):
    ...
    portrait_url: Mapped[str | None] = mapped_column(
        String(512), nullable=True, comment="2D立绘/头像URL"
    )
    ...
    voice_speaker_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    primary_provider_config_id: Mapped[int | None] = mapped_column(
        Integer,
    ...
    avatar: Mapped[Optional["Avatar"]] = relationship(
        "Avatar", back_populates="characters"
    )
    voice_asset: Mapped[Optional["VoiceAsset"]] = relationship(
        "VoiceAsset", back_populates="characters"
    )
    primary_model: Mapped[Optional["Model"]] = relationship(
        "Model", back_populates="characters"
    )
...
class CharacterMotionBinding(Base):
    ...
    character: Mapped["Character"] = relationship(
        "Character", back_populates="motion_bindings"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "character_id", "motion_id", "category", name="uq_character_motion_category"
        ),
