...
from core.models.config import ModelConfig, ProviderConfig
from core.models.factory import ModelFactory
from core.repositories import CharacterRepository, ModelRepository, ProviderRepository
...
class ModelService:
    ...
    def create_model_instance(
        self, provider_config_id: int, model_id: str, **kwargs
    ) -> Any:
        """创建模型实例
        ...
        model_orm = self.model_repo.get_by_provider_and_model(
        ...
        try:
            ...
            if model is None:
                ...
                raise ValueError(
                    f"模型 {provider_config_id}/{model_id} 创建失败，请检查配置"
                )
            ...
            raise ValueError(
                f"创建模型 {provider_config_id}/{model_id} 时出错: {str(e)}"
            ) from e
    ...
    def check_provider_dependencies(self, provider_config_id: int) -> dict:
        ...
        """
        dependencies = {"models": [], "characters": []}

        # 检查依赖的模型
        models = self.model_repo.list_by_provider(
            provider_config_id, enabled_only=False
        )
