        ...
        data = [
            {
                ...
                "available_expressions": (
                    json.loads(avatar.available_expressions)
                    if avatar.available_expressions
                    else []
                ),
...
async def get_avatar(avatar_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    try:
        ...
        data = {
            ...
            # 获取引用该形象的角色
            "referenced_by": [
                {"id": char.id, "name": char.name} for char in avatar.characters
            ],
        }
...
async def update_avatar(
    ...
    try:
        ...
        return {"code": 200, "message": "形象更新成功", "data": data}
...
async def upload_avatar(
    ...
    try:
        ...
        # 构建响应
        expressions_list = (
            json.loads(available_expressions) if available_expressions else []
        )
    ...
    try:
        ...
        try:
            ...
            if model_file_path.exists():
                ...
                logger.warning(
                    "模型文件不存在，跳过删除", extra={"path": str(model_file_path)}
                )
