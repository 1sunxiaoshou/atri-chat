...
class TTSSynthesizeRequest(BaseModel):
    ...
    character_id: str | None = None  # 可选：指定角色使用其绑定的音色
    ...
    try:
        ...
        if not enabled_providers:
            return {
                "code": 200,
                "message": "TTS 未配置",
                "data": {"enabled": False, "providers": [], "voices": []},
            }
...
async def synthesize_speech(
    ...
    try:
        ...
        if req.character_id:
            character = (
                db.query(Character).filter(Character.id == req.character_id).first()
            )
        ...
        if stream:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="TTS生成失败：无数据"
                ) from None
            except Exception as e:
                logger.error(f"获取采样率失败: {e}")
                raise HTTPException(status_code=500, detail="TTS 采样率配置有误") from e
        ...
        raise HTTPException(status_code=400, detail="配置参数无效") from e
    except Exception as e:
