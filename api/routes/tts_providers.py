...
@router.get(
    "/{provider_id}", summary="获取 TTS 供应商详情", response_model=ResponseModel
)
...
async def update_provider(
    ...
    try:
        ...
        return {"code": 200, "message": "TTS 供应商更新成功", "data": data}
...
@router.delete(
    "/{provider_id}", summary="删除 TTS 供应商", response_model=ResponseModel
)
    ...
    try:
        ...
        if voice_ids:
            ...
            referenced_characters = (
                db.query(Character)
                .filter(Character.voice_asset_id.in_(voice_ids))
                .all()
            )
        ...
        return {"code": 200, "message": "TTS 供应商删除成功", "data": None}
...
async def list_provider_types() -> dict[str, Any]:
    ...
    return {"code": 200, "message": "获取成功", "data": types}


@router.get(
    "/types/{provider_type}/template",
    summary="获取供应商配置模板",
...
@router.post(
    "/{provider_id}/test", summary="测试 TTS 供应商配置", response_model=ResponseModel
)
async def test_provider(
    ...
    try:
        ...
        tts_instance = factory.create_tts(
            provider_type=provider.provider_type, config=provider.config_payload
        )
