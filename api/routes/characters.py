...
from core.db import Character, CharacterMotionBinding, Motion, VoiceAsset
from core.db.utils import (
    InvalidReferenceError,
    validate_avatar_exists,
    validate_model_exists,
    validate_voice_asset_exists,
)
...
class CharacterCreate(BaseModel):
    ...
    system_prompt: str = Field(
        "你是一个友好、乐于助人的AI助手。", description="系统提示词"
    )
    portrait_url: str | None = Field(None, description="2D立绘/头像URL")
    avatar_id: str | None = Field(None, description="3D形象资产ID（可选）")
    voice_asset_id: int | None = Field(None, description="音色资产 ID（可选）")
    voice_speaker_id: str | None = Field(None, description="音色说话人 ID（可选）")
    primary_model_id: int | None = Field(None, description="主模型 ID（可选）")
    primary_provider_config_id: int | None = Field(
        None, description="主供应商配置 ID（可选）"
    )
    enabled: bool = Field(True, description="是否启用")
    motion_bindings: list[MotionBindingInput] | None = Field(
        None, description="动作绑定列表（可选）"
    )
...
class CharacterUpdate(BaseModel):
    ...
    voice_asset_id: int | None = Field(None, description="音色资产 ID")
    voice_speaker_id: str | None = Field(None, description="音色说话人 ID")
    primary_model_id: int | None = Field(None, description="主模型 ID")
    primary_provider_config_id: int | None = Field(None, description="主供应商配置 ID")
        ...
        data = [
            {
                ...
                "voice_provider_type": (
                    char.voice_asset.provider.provider_type
                    if char.voice_asset
                    else None
                ),
...
@router.get(
    "/characters/{character_id}", summary="获取角色详情", response_model=ResponseModel
)
...
async def create_character(
    character_create: CharacterCreate, db: Session = Depends(get_db)
) -> dict[str, Any]:
    """创建角色（支持同时创建动作绑定）"""
    try:
        ...
        try:
            ...
            if character_create.motion_bindings:
                for binding in character_create.motion_bindings:
                    motion = (
                        db.query(Motion).filter(Motion.id == binding.motion_id).first()
                    )
                    if not motion:
...
@router.patch(
    "/characters/{character_id}", summary="更新角色", response_model=ResponseModel
)
async def update_character(
    ...
    try:
            ...
            if old_portrait_url:
                try:
                    ...
                    if other_users == 0:
                        ...
                        logger.info(
                            f"旧立绘文件被 {other_users} 个其他角色使用,跳过删除"
                        )
                except Exception as e:
        ...
        return {"code": 200, "message": "角色更新成功", "data": data}
...
@router.delete(
    "/characters/{character_id}", summary="删除角色", response_model=ResponseModel
)
    ...
    try:
        ...
        if portrait_to_delete:
            try:
                ...
                other_users = (
                    db.query(Character)
                    .filter(Character.portrait_url == portrait_to_delete)
                    .count()
                )
