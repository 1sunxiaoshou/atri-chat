...
from core.db import Character, Conversation, Message
...
async def list_conversations(
    ...
    try:
        ...
        query = db.query(Conversation).options(
            joinedload(Conversation.character), selectinload(Conversation.messages)
        )
...
async def get_conversation(
    conversation_id: str, include_messages: bool = False, db: Session = Depends(get_db)
) -> dict[str, Any]:
    """获取会话详情"""
    try:
        from sqlalchemy.orm import joinedload, selectinload
...
async def create_conversation(
    conversation_create: ConversationCreate, db: Session = Depends(get_db)
) -> dict[str, Any]:
    """创建会话"""
    try:
        # 验证角色是否存在
        character = (
            db.query(Character)
            .filter(Character.id == conversation_create.character_id)
            .first()
        )
...
@router.put(
    "/conversations/{conversation_id}", summary="更新会话", response_model=ResponseModel
)
...
@router.delete(
    "/conversations/{conversation_id}", summary="删除会话", response_model=ResponseModel
)
...
async def get_conversation_messages(
    conversation_id: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
) -> dict[str, Any]:
    """获取会话的所有消息"""
    try:
        ...
        raise HTTPException(status_code=500, detail="获取消息失败") from e
...
async def clear_conversation_messages(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="清空失败") from e
