...
export const CharacterEditor: React.FC<CharacterEditorProps> = ({
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full bg-background overflow-hidden">
            {/* Left: Preview Area */}
            <div className="relative flex-1 bg-slate-900 overflow-hidden min-h-[40vh] lg:min-h-0">
                {/* Top Actions */}
                <div className="absolute top-3 left-3 sm:top-6 sm:left-6 z-10">
                    <button
                        onClick={onBack}
                        className="flex items-center gap-1 sm:gap-2 text-white/70 hover:text-white transition-colors bg-black/20 backdrop-blur-lg px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-white/10 text-sm"
                    >
                        <ChevronLeft size={16} className="sm:w-[18px] sm:h-[18px]" strokeWidth={2.5} />
                        <span className="hidden sm:inline">{t('admin.back') || '返回'}</span>
                    </button>
                </div>
                ...
                <div className="w-full h-full flex items-center justify-center p-4">
                    <div className="text-center">
                        {/* Avatar Display */}
                        <div className="w-32 h-32 sm:w-48 sm:h-48 lg:w-64 lg:h-64 mx-auto rounded-3xl overflow-hidden ring-4 ring-white/10 shadow-2xl bg-slate-800 mb-4 sm:mb-6">
                            <img
                                src={buildAvatarUrl(character.avatar)}
                                alt={character.name}
                        ...
                        {/* Character Info */}
                        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-2">{character.name || 'Unnamed Character'}</h2>
                        <p className="text-white/50 text-xs sm:text-sm font-mono tracking-widest uppercase">
                ...
                {animations.length > 0 && (
                    <div className="absolute bottom-4 sm:bottom-10 left-1/2 -translate-x-1/2 flex gap-1 sm:gap-2 bg-black/40 backdrop-blur-xl p-1.5 sm:p-2 rounded-2xl border border-white/10 max-w-[90%] overflow-x-auto">
                        {animations.slice(0, 6).map(anim => (
                            <button
                                key={anim.animation_id}
                                className="px-2 py-1 sm:px-4 sm:py-2 text-[10px] sm:text-xs font-bold text-white/80 hover:bg-white/20 rounded-xl transition-all whitespace-nowrap"
                                title={anim.name}
                            >
            ...
            {/* Right: Config Area */}
            <div className="w-full lg:w-[500px] xl:w-[600px] relative border-t lg:border-t-0 lg:border-l border-border shadow-2xl z-20 bg-card h-[60vh] lg:h-full flex flex-col">
                {/* Tabs Nav */}
                <div className="flex border-b border-border px-2 sm:px-4 flex-shrink-0 overflow-x-auto">
                    {tabs.map(tab => {
                        const Icon = tab.icon;
                        return (
                            <button
                                ...
                                className={cn(
                                    "flex-1 min-w-[80px] flex flex-col items-center py-3 sm:py-4 gap-1 text-xs font-bold border-b-2 transition-all whitespace-nowrap",
                                    activeTab === tab.id
                                        ? "border-primary text-primary"
                                        : "border-transparent text-muted-foreground hover:text-foreground"
                                )}
                            >
                                <Icon size={18} className="sm:w-5 sm:h-5" strokeWidth={activeTab === tab.id ? 2.5 : 2} />
                ...
                {/* Tab Content */}
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar pb-24" style={{ minHeight: 0 }}>
                    {/* Persona Tab */}
                    {activeTab === 'persona' && (
                ...
                {/* Footer Actions */}
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 lg:p-6 border-t border-border flex gap-2 sm:gap-3 bg-card">
                    <Button
                        variant="outline"
                        onClick={onBack}
                    ...
                    <Button
                        ...
                        className="flex-1 text-sm sm:text-base"
                    >
                        <Save size={16} className="sm:w-[18px] sm:h-[18px] mr-1 sm:mr-2" />
