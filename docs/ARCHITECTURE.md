...
本文档详细阐释了 ATRI Chat 项目的系统架构，旨在帮助开发人员理解该项目从底层交互到上层表现形式的骨架及其设计理念。
...
当项目经由 `build.py` 被打包为桌面 `.exe` 格式时：
- Tauri 框架会将本地网页环境承载在内嵌浏览器框架（Webview2 等）当中运行。
- `pyinstaller` 将 Python 后端与 `sherpa-onnx` 等环境打包至一个无需用户独立安装 Python 的隔离独立包 (`binaries/`) 内。
